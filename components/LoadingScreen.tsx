  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        background: 'repeating-linear-gradient(0deg, transparent, transparent 29px, rgba(0,229,255,.03) 30px)',
        pointerEvents: 'none',
      }} />
      ...
      {/* 상태 텍스트 */}
      <div style={{ fontSize: '9px', color: 'var(--tx2)', letterSpacing: '.2em' }}>
        SYS://PORTFOLIO.INIT
      </div>

      {/* 메인 텍스트 */}
      <div style={{ textAlign: 'center' }}>
        <div style={{ fontSize: '11px', color: 'var(--tx2)', letterSpacing: '.14em', marginBottom: '.5rem' }}>
          &lt;/
        </div>
        <div style={{ fontSize: '52px', fontWeight: 700, color: 'var(--txw)', letterSpacing: '.04em', lineHeight: 1 }}>
          Cédrai
        </div>
        <div style={{ fontSize: '11px', color: 'var(--tx2)', letterSpacing: '.14em', marginTop: '.5rem' }}>
          &gt;
        </div>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '.6rem', width: '240px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', width: '100%' }}>
          ...
          <span style={{ fontSize: '9px', color: 'var(--acc)', letterSpacing: '.08em' }}>{progress}%</span>
        </div>
        ...
        <div style={{ display: 'flex', gap: '3px', width: '100%' }}>
          {[...Array(20)].map((_, i) => (
            <div
              ...
              style={{
                flex: 1, height: '3px',
                background: i < Math.floor(progress / 5) ? 'var(--acc)' : 'var(--bd)',
